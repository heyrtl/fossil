...
// .wrangler/tmp/bundle-iq1uBk/strip-cf-connecting-ip-header.js
...
// node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260412.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260412.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
// node_modules/.pnpm/@openfossil+core@0.1.0/node_modules/@openfossil/core/dist/schema.js
var FOSSIL_ID_PATTERN = /^fossil_[a-f0-9]{12}$/;
function generateId() {
...
function validateRecord(record) {
  if (typeof record !== "object" || record === null)
    return false;
  const r = record;
  if (!FOSSIL_ID_PATTERN.test(r.id))
    return false;
  if (r.protocol_version !== "1.0")
    return false;
  if (typeof r.timestamp !== "string")
    return false;
  if (typeof r.agent !== "object" || r.agent === null)
    return false;
  if (typeof r.situation !== "object" || r.situation === null)
    return false;
  if (typeof r.failure !== "object" || r.failure === null)
    return false;
  if (typeof r.resolution !== "object" || r.resolution === null)
    return false;
  return true;
}
...
// src/lib/store.ts
async function insertFossil(db, record, embedding) {
...
__name(insertFossil, "insertFossil");
async function searchFossils(db, queryEmbedding, topK, minScore, domain2) {
  let query = "SELECT * FROM fossils";
  const bindings = [];
  if (domain2) {
    query += " WHERE task_domain = ?";
    bindings.push(domain2);
  }
  ...
  const scored = results.map((row) => {
    ...
    return { row, score };
  }).filter(({ score }) => score >= minScore).sort((a, b) => b.score - a.score).slice(0, topK);
  return scored.map(({ row, score }) => ({
    record: rowToRecord(row),
    score: Math.round(score * 1e4) / 1e4
  }));
}
__name(searchFossils, "searchFossils");
async function getFossil(db, id) {
  const row = await db.prepare("SELECT * FROM fossils WHERE id = ?").bind(id).first();
  if (!row)
    return null;
  return rowToRecord(row);
}
__name(getFossil, "getFossil");
async function deleteFossil(db, id) {
  const result = await db.prepare("DELETE FROM fossils WHERE id = ?").bind(id).run();
  return (result.meta.changes ?? 0) > 0;
}
__name(deleteFossil, "deleteFossil");
async function listFossils(db, limit, offset) {
  const { results } = await db.prepare(
    "SELECT * FROM fossils ORDER BY timestamp DESC LIMIT ? OFFSET ?"
  ).bind(limit, offset).all();
  return results.map((row) => rowToRecord(row));
}
__name(listFossils, "listFossils");
async function countFossils(db) {
  const row = await db.prepare("SELECT COUNT(*) as count FROM fossils").first();
  return row?.count ?? 0;
}
__name(countFossils, "countFossils");
...
records.post("/", async (c) => {
  ...
  if (body.id && body.protocol_version) {
    if (!validateRecord(body))
      return c.json({ error: "invalid fossil record" }, 400);
    record = body;
  } else {
    if (!body.situation || !body.failure_type || !body.failure || !body.severity || !body.resolution_type || !body.resolution || !body.framework || !body.model) {
...
search.get("/", async (c) => {
  ...
  const domain2 = c.req.query("domain") ?? void 0;
  const queryEmbedding = await embed(c.env.AI, q);
  const results = await searchFossils(c.env.DB, queryEmbedding, topK, minScore, domain2);
...
app.get(
  "/health",
  (c) => c.json({ status: "ok", version: "0.1.0", timestamp: (/* @__PURE__ */ new Date()).toISOString() })
);
...
// node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260412.1/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260412.1/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-iq1uBk/middleware-insertion-facade.js
...
// node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260412.1/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-iq1uBk/middleware-loader.entry.ts
